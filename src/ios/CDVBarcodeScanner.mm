...
- (void)returnSuccess:(NSString*)scannedText format:(NSString*)format cancelled:(BOOL)cancelled flipped:(BOOL)flipped usePrevious:(BOOL)usePrevious callback:(NSString*)callback;
...
@property (nonatomic, retain) NSString*                   overlayButtonText;
...
- (id)initWithPlugin:(CDVBarcodeScanner*)plugin callback:(NSString*)callback parentViewController:(UIViewController*)parentViewController
    alterateOverlayXib:(NSString *)alternateXib overlayButtonText:(NSString *)overlayButtonText;
- (void)scanBarcode;
- (void)barcodeScanSucceeded:(NSString*)text format:(NSString*)format;
- (void)barcodeScanFailed:(NSString*)message;
- (void)barcodeScanCancelled;
- (void)barcodeScanUsePrevious;
- (void)openDialog;
...
- (void)scan:(CDVInvokedUrlCommand*)command {
    ...
    NSString *overlayButtonText = nil;
    if ( [command.arguments count] > 0 )
    {
        overlayXib = [command.arguments objectAtIndex:0];
        if ( [command.arguments count] > 1 )
        {
            overlayButtonText = [command.arguments objectAtIndex:1];
        }
    }
...
- (void)returnSuccess:(NSString*)scannedText format:(NSString*)format cancelled:(BOOL)cancelled flipped:(BOOL)flipped usePrevious:(BOOL)usePrevious callback:(NSString*)callback{
    ...
    NSNumber* usePrevNumber = [NSNumber numberWithInt:(usePrevious?1:0)];

    NSMutableDictionary* resultDict = [[NSMutableDictionary alloc] init];
    [resultDict setObject:scannedText     forKey:@"text"];
    [resultDict setObject:format          forKey:@"format"];
    [resultDict setObject:cancelledNumber forKey:@"cancelled"];
    [resultDict setObject:usePrevNumber forKey:@"usePrevious"];
...
@synthesize overlayButtonText    = _overlayButtonText;
...
parentViewController:(UIViewController*)parentViewController
  alterateOverlayXib:(NSString *)alternateXib overlayButtonText:(NSString *)overlayButtonText {
    ...
    self.overlayButtonText    = overlayButtonText;
...
//    self.previewLayer = nil;
    ...
    self.viewController = [[CDVbcsViewController alloc] initWithProcessor: self alternateOverlay:self.alternateXib overlayButtonText:self.overlayButtonText];
...
- (void)barcodeScanSucceeded:(NSString*)text format:(NSString*)format {
    dispatch_sync(dispatch_get_main_queue(), ^{
        ...
        [self.plugin returnSuccess:text format:format cancelled:FALSE flipped:FALSE usePrevious:FALSE callback:self.callback];
    });
}
...
- (void)barcodeScanCancelled {
    [self barcodeScanDone];
    [self.plugin returnSuccess:@"" format:@"" cancelled:TRUE flipped:self.isFlipped usePrevious:FALSE callback:self.callback];
    if (self.isFlipped) {
        self.isFlipped = NO;
    }
}
...
- (void)barcodeScanUsePrevious {
    [self barcodeScanDone];
    [self.plugin returnSuccess:@"" format:@"" cancelled:FALSE flipped:self.isFlipped usePrevious:TRUE callback:self.callback];
    if (self.isFlipped) {
        self.isFlipped = NO;
    }
}
...
//--------------------------------------------------------------------------
- (id)initWithProcessor:(CDVbcsProcessor*)processor alternateOverlay:(NSString *)alternateXib overlayButtonText:(NSString *)overlayButtonText {
    self = [super init];
    if (!self) return self;
...
- (IBAction)continueButtonPressed:(id)sender {
    [self.processor performSelector:@selector(barcodeScanUsePrevious) withObject:nil afterDelay:0];
}
...
- (UIView*)buildOverlayView {
    ...
    {
        NSLog(@"%@", @"Using alternate overlay.");
        NSLog(@"%@", self.overlayButtonText);
